	...
	if (window.Store){
		...
		for (q = 0; q < countContact; q++){	
			...
			if (Store.Contact._models[q].__x_id.server == "c.us" && Store.Contact._models[q].__x_type === "in"){
				...
				data = new Date(); data= data.getDate() + "." + (data.getMonth()+1) + "." + data.getFullYear ();
				Qt = numero.length;				
				if (Qt == 13){
				...
				ListChats[cont] = 'Nome: ' + nome +';numero: ' + numero  + 'Data:' + data.getDate; 					
				cont++
		...
		printTxt(ListChats, 'Lista de Envio' + ' - ' +  data + '.csv')
		ListChats = new Array();
